    ...
    public class NFCReader
    {
        ...
        public string GetCardUID()
        {
            ...
            else
            {
                ...
                string[] dataUID = cardUID.Split('-');
                string okUID = "";
                for (int indexUID = dataUID.Length - 1; indexUID >= 0; indexUID--)
                {
                    okUID += dataUID[indexUID];
                }
                cardUID = okUID;*/
                //Reverse UID
                cardUID = BitConverter.ToString(receivedUID.Take(4).ToArray()).ToLower();
                //Formato UID 
                string[] dataUID = cardUID.Split('-');
                string okUID = "";
                for (int indexUID = dataUID.Length - 1; indexUID >= 0; indexUID--)
